...
const Header = () => {
  ...
  return (
    <header className="header_wrapper d-flex position-sticky modal-dialog-centered">
      <Link
        to="/"
        className=" nav-logo-link nav-progressive-attribute CLear_Link_Borderb navbar-expand"
        aria-label="Amazon"
      >
        <img
          src={logo}
          alt="Amazon Logo"
          className="header_logo pb-1 ms-2 me-3 bg-black"
        />
      </Link>
        ...
        <div className="header_nav d-flex">
          <div className="header_option header_optionlocation d-flex mx-2 text-white text-center me-2">
            ...
            <div className="d-flex">
              <LocationOnOutlinedIcon className="text-white header_location" />
              <span className="header_optionLinetwo fw-bolder">Ethiopia</span>
            </div>
          </div>
        </div>
      </Link>
      ...
      <div className="header_search d-flex">
        <input
          type="text"
          className="nav-input nav-progressive-attribute header_searchinput  modal-dialog-centered p-2"
        />
        <SearchSharpIcon className="header_searchIcon p-1 bg-warning" />
      </div>
      <Link
        to="/"
        className="nav-logo-link navbar-toggler nav-progressive-attribute"
      >
        <div className="amercia_logo d-flex CLear_Link_Border">
          <img
            className="america_logo my-1 ms-1 me-0 mx-3"
            src={America}
            alt="America Logo"
          />
          <select
            className="modal-dialog-centered header_optionChocice"
            style={{ backgroundColor: "white", height: "5px", width: "5px" }}
          >
      ...
      <div className="header_nav d-flex">
        ...
        >
          <div
            className="header_option d-flex mx-2 text-white CLear_Link_Border"
            onClick={Signout}
          >
            <span
              className={`${
                !user
                  ...
                  : "text-center fs-5 btn rounded-6 text-white bg-warning pb-1"
              } `}
            >
              {!user ? "Hello Guest" : user?.email[0].toUpperCase()}
            </span>
            {/* user ? "Guest" : user?.email */}
            <span className="header_optionLinetwo fw-bolder">
              {user ? "" : "Sign In"}
            </span>
          </div>
        </Link>
      ...
      >
        <div className="nav-cart-count-container pb-0 position-relative float-none header_optionbasket modal-dialog-centered CLear_Link_Border">
          <span className="nav-cart-icon nav-sprite">
            <ShoppingCartOutlinedIcon className="text-white the_cart" />
          </span>
          <span className="nav-cart-count nav-cart-count nav-cart-0 nav-progressive-attribute nav-progressive-content text-warning fw-bolder p-2 header_optionLinetwo header_basketcount mx-1 fs-4">
            {basket.length}
          </span>
        </div>
      </Link>
